            ...
            for i in range(min(size,sizeo)):
                for j in range(min(size,sizeo)):
                    if str(mess['#@'+str(i)]+':'+mess['#o'+str(i)]) in index_of_author and \
                                            str(mess['#@' + str(j)] + ':' + mess['#o' + str(j)]) in index_of_author and \
                                            int(index_of_author[mess['#@' + str(i)] + ':' + mess['#o' + str(i)]]) < int(index_of_author[mess['#@' + str(j)] + ':' + mess['#o' + str(j)]]):
...
fr = open('./inter_res/network_dict_sorted.json', 'w',encoding = 'utf-8',errors='ignore')
